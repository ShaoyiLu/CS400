...
public class CS400Graph<NodeType,EdgeType extends Number> implements GraphADT<NodeType,EdgeType> {
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
        ...
        if(!vertices.containsKey(start)){
            throw new NoSuchElementException("there is an empty start vertex");
        }
        Vertex getStart = vertices.get(start);
        vertexList.add(getStart);

        if(!vertices.containsKey(end)){
            throw new NoSuchElementException("there is an empty end vertex");
        }
        Vertex getEnd = vertices.get(end);

        PriorityQueue<Path> pqPath = new PriorityQueue<>();
        for(int i = 0; i < getStart.edgesLeaving.size(); i++){
            Path temp = new Path(getStart);
            Path startPath = new Path(temp, getStart.edgesLeaving.get(i));
            pqPath.add(startPath);
        }

        while(!pqPath.isEmpty()){
            Path rightPath = pqPath.poll();
            Vertex visited = rightPath.end;
            vertexList.add(visited);
            if(visited == getEnd){
                return rightPath;
            }
            for (int i = 0; i < visited.edgesLeaving.size(); i++) {
                if (!vertexList.contains(visited.edgesLeaving.get(i).target)) {
                    Path temp = new Path(rightPath, visited.edgesLeaving.get(i));
                    pqPath.add(temp);
                }
            }
        }
        throw new NoSuchElementException("there is no vertex or edge in the graph");
    }
